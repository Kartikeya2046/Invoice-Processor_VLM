...
- **quantity**: A positive number (count of units). Zero or negative is implausible. There is no fixed "normal" range for quantity — only flag if it is zero, negative, or non-numeric. Do not lower confidence just because a number seems large or small to you.
- **unit_price**: A positive number, typically with up to 2 decimal places. Zero or negative is implausible. Should NOT contain currency symbols or commas (those should already be stripped — if you see them, flag it). IMPORTANT: There is NO fixed "normal" price range — unit prices can legitimately range from under 1 to many thousands depending on the item. Do NOT flag a unit_price merely for being "outside an expected range" or "unusually high/low" unless it is zero, negative, or clearly malformed (e.g. contains text). A specific numeric value like 55.7 or 12000.0 is plausible on its own and should receive confidence 1.0.
...
Only include field_name entries for fields actually present in the input JSON. For "invoice" document_type, scan among: po_number, supplier, invoice_number, invoice_date, quantity, unit_price, cgst, sgst. For "bill_of_entry" document_type, scan among: boe_number, boe_date, igst, cust_duty, sbcess."""
